...
class BST(bt.BT):
    ...
    def delete(self, v):
        ...
        '''
        parent_node=self.find_member(v)

        if parent_node.lc().value() == v: #kollar om noden som ska bort är ett höger eller vänsterlöv
            node=parent_node.lc()
        else:
            node=parent_node.rc()
        ...
        if node.lc().value() == None and node.rc().value() == None: #Lövnod
            ...
            if parent_node.lc().value() == v:
                parent_node.set_lc(node.lc())
            else:
                parent_node.set_rc(node.lc())
            return self

        elif node.lc().value() == None and node.rc().value() != None: # om noden som ska bort bara har högerbarn
            if parent_node.lc().value() == v:
                parent_node.set_lc(node.rc())
            else:
                parent_node.set_rc(node.rc())
            return self
