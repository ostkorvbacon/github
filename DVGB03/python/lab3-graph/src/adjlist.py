...
class AdjacencyList:
    ...
    # Node operations
    ###
    def insert(self, name, insertNode):
        head = self
        while not self.head().is_empty():
            if name < self.name():
                if self == head: # Insert is head
                    head = insertNode
                    insertNode._tail = self
                    return head
                else: # Insert is not head
                    temp = parentNode._tail
                    parentNode._tail = insertNode
                    insertNode._tail = temp
                    return head
            parentNode = self
            self = self.tail()

    def balance(self, name, insertNode, parentNode): # Balances if unbalanced else returns head
        if parentNode.name() > insertNode.name():
            parentNode._tail = insertNode._tail
            self = self.insert(name, insertNode)
    ...
    def add_node(self, name, info=None):
        ...
        while not self.head().is_empty():
            parentNode = self
            self = self.tail()
        parentNode.cons(self)
        self.__init__(name, info)
        insertNode = self
        return head.balance(name, insertNode, parentNode) # egen funktion


    def delete_node(self, head, name):
        '''
    ...
    def findNode(self, name): # Finds node with name "name" and returns that node
    ...
    def _add_edge(self, src, dst, weight):
        ...
        while self.name() != dst:
            self = self.tail()
        srcNode = self
        srcEdge = Edge.add(self, dst, weight)
        print("Edge value", srcEdge.dst())
        print("Edge value", srcEdge.weight())
        srcNode.set_edges(srcEdge)
        print("src val" , srcNode.edges().dst())
        print("src weight" , srcNode.edges().weight())
...
class Edge:
    ...
    # Edge Operations
    ###
    def insertEdge(self, name, insertEdge):
        head = self
        while not self.head().is_empty():
            if name < self.dst():
                if self == head: # Insert is head
                    head = insertEdge
                    insertEdge._tail = self
                    return head
                else: # Insert is not head
                    temp = parentEdge._tail
                    parentEdge._tail = insertEdge
                    insertEdge._tail = temp
                    return head
            parentEdge = self
            self = self.tail()

    def balanceEdge(self, name, insertEdge, parentEdge): # Balances if unbalanced else returns head
        if parentEdge.dst() > insertEdge.dst():
            parentEdge._tail = insertEdge._tail
            self = self.insertEdge(name, insertEdge)
    ...
    def add(self, dst, weight=1):
        ...
        while not self.head().is_empty():
            parentEdge = self
            self = self.tail()
        parentEdge.cons(self)
        self.__init__(dst, weight)

        return self.balanceEdge(dst, head, parentEdge) # egen funktion
